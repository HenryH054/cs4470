...
class InferenceModule:
    ...
    def getObservationProb(
        ...
        if noisy_distance is None:
            return float(ghost_position == jail_position)
        if ghost_position == jail_position:
            return float(noisy_distance is None)
        estimatedDistance = manhattanDistance(pacman_position, ghost_position)
        return busters.getObservationProbability(noisy_distance, estimatedDistance)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, game_state: Any) -> None:
        ...
        for position in self.allPositions:
            pr_transition[position] = self.getPositionDistribution(game_state, position)
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                nextPosDistribution = self.getPositionDistribution(
                    game_state, oldParticle, i, self.ghostAgents[i])
