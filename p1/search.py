...
from typing import List, Tuple, Any, Optional, Dict, Callable, final
...
def depthFirstSearch(problem: 'SearchProblem') -> List[str]:
    ...
    visited = []
    actionList = []
    dfs.push((problem.getStartState(), actionList))
    while dfs:
        node, actions = dfs.pop()
        if not node in visited:
            visited.append(node)
            if problem.isGoalState(node):
                return actions
            for successor in problem.getSuccessors(node):
                coordinate, direction, _ = successor
                nextActions = actions + [direction]
                dfs.push((coordinate, nextActions))
    return []
...
def breadthFirstSearch(problem: 'SearchProblem') -> List[str]:
    ...
    visited = []
    actionList = []
    bfs.push((problem.getStartState(), actionList))
    while bfs:
        node, actions = bfs.pop()
        if not node in visited:
            visited.append(node)
            if problem.isGoalState(node):
                return actions
            for successor in problem.getSuccessors(node):
                coordinate, direction, _ = successor
                nextActions = actions + [direction]
                bfs.push((coordinate, nextActions))
    return []
...
def uniformCostSearch(problem: 'SearchProblem') -> List[str]:
    ...
    while not ucs.isEmpty():
        currentNode = ucs.pop()
        state, path = currentNode[0], currentNode[1]
        ...
        for successor, action, stepCost in problem.getSuccessors(state):
            ...
            ucs.push([successor, newPath], cost)           
...
def aStarSearch(problem: 'SearchProblem', heuristic: Callable = nullHeuristic) -> List[str]:
    ...
    visited = []
    actionList = []
    aStar.push((problem.getStartState(), actionList), heuristic(problem.getStartState(), problem))
    while aStar:
        node, actions = aStar.pop()
        if not node in visited:
            visited.append(node)
            if problem.isGoalState(node):
                return actions
            for successor in problem.getSuccessors(node):
                ...
                aStar.push((coordinate, nextActions), nextCost)
    return []
